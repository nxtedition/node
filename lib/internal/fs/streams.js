...
let StringDecoder;
async function* readIterable(path, options, readable) {
  options = options || {};
  ...
  if (fsImpl.read && typeof fsImpl.read !== 'function') {
    throw new ERR_INVALID_ARG_TYPE('options.fs.read', 'function',
                                   fsImpl.read);
  }

  if (fsImpl.writev && typeof fsImpl.writev !== 'function') {
    throw new ERR_INVALID_ARG_TYPE('options.fs.writev', 'function',
                                   fsImpl.writev);
  }

  if (typeof fsImpl.close !== 'function') {
    throw new ERR_INVALID_ARG_TYPE('options.fs.close', 'function',
                                   fsImpl.close);
  }

  const openAsync = internalUtil.promisify(fs.open);
  const readAsync = internalUtil.promisify(fs.read);
  const closeAsync = internalUtil.promisify(fs.close);
  ...
  try {
    ...
    while (true) {
      ...
      pos += bytesRead;
      if (readable) {
        readable.bytesRead += bytesRead;
      }
      buf = buf.slice(0, bytesRead);

      if (decoder) {
        yield decoder.write(buf);
      } else {
        yield buf;
      }
    }
